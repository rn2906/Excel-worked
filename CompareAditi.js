...
// Load the Excel file
const excelFilePath = "Aditi.xlsx";
const workbook = xlsx.readFile(excelFilePath);
const sheetName = workbook.SheetNames[0];
const excelData = xlsx.utils.sheet_to_json(workbook.Sheets[sheetName]);

// Load the JSON file
const jsonFilePath = "Aditi.json";
const jsonData = JSON.parse(fs.readFileSync(jsonFilePath, "utf8"));
...
const config = {
  ...
  dateFields: ["Maturity Date", "Issue Date", "Trade Date", "Settlement Date"], // Explicitly specify date fields
};
...
// Extract unique account numbers from JSON data
const accountNumbers = new Set(
  jsonData.map((record) => String(record.NUMBER).trim())
);
console.log("Extracted Account Numbers from JSON Data:", [...accountNumbers]);
...
// Filter Excel data to keep only records with those account numbers
const filteredExcelData = excelData.filter((row) =>
  accountNumbers.has(String(row["Account"]).trim())
);
...
// Find and print differences
filteredExcelData.forEach((excelRow) => {
  const transactionId = excelRow["Transaction No"];
  jsonData.forEach((jsonRecord) => {
    if (
      jsonRecord.Transaction == transactionId &&
